...
func TestBuildAIShadowReportRequiresExplicitRFC3339Window(t *testing.T) {
	...
	report, err := buildAIShadowReport(database, start.Format(time.RFC3339), end.Format(time.RFC3339), recommendation.StrategyVersion)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := buildAIShadowReport(database, "2026-08-01", end.Format(time.RFC3339), recommendation.StrategyVersion); err == nil || !strings.Contains(err.Error(), "RFC3339") {
...
func TestBuildAIShadowReportRejectsUnavailableDatabase(t *testing.T) {
	start := time.Now().UTC()
	if _, err := buildAIShadowReport(nil, start.Format(time.RFC3339), start.Add(time.Hour).Format(time.RFC3339), recommendation.StrategyVersion); err == nil {
