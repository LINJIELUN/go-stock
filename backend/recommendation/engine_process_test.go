...
func engineFixture(t *testing.T) (models.AIAnalysisJob, EngineInputContext, []byte) {
	...
	frozen := frozenAnalysisInput{SchemaVersion: InputBundleSchemaVersion, StockCode: job.StockCode, ValidationBatchID: job.ValidationBatchID,
		...
		DailyBars: []FrozenDailyBar{{EvidenceID: dailyBarEvidenceID(job.StockCode, now.Add(-24*time.Hour)), TradeDate: now.Add(-24 * time.Hour), Open: 9.8, High: 10.2, Low: 9.7, Close: 10, Volume: 100, Turnover: 1000, Source: "validated"}},
